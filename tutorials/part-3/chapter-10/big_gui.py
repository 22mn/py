...
class Hello(GuiMixin, GuiMakerWindowMenu):
	def start(self):
		...
		self.menuBar = [("File", 0, 
			...
			("Edit", 0, [("Cut", -1, self.notdone),
				("Paste", -1, self.fileOpen),
				"separator",
				("Stuff",-1,[("Clone", -1, self.notdone),
					("More", -1, self.more)]),
				("Delete", -1, lambda:0),[5]]),
			("Play", 0, [("Hello", 0, self.greeting),
				("Popup...", 0, self.dialog), 
				("Demos", 0,
					[("Toplevels", 0, lambda: self.spawn(r"../chapter-8/toplevel2.py")),
					("Frames", 0, lambda: self.spawn(r"../chapter-8/demoAll-frm.py")),
					("Images", 0, lambda: self.spawn(r"../chapter-8/buttonpics.py")),
					("Alarm", 0, lambda: self.spawn(r"../chapter-9/alarm.py")),
	...
	def dialog(self):
		...
		# false is index 0, true won't call
	...
	def more(self):
		new = Toplevel()
		Label(new, text="A new non-modeal window").pack()
